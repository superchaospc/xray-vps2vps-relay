...
本项目仅用于学习网络协议、Linux 运维自动化和自有服务器链路管理。请在部署和使用前确认你的使用场景符合所在国家/地区的法律法规、服务商条款和网络管理要求。

使用者需自行承担因部署、配置、网络访问、服务器安全、账号封禁、流量费用、数据丢失或其他风险造成的后果。项目作者不对任何直接或间接损失负责，也不提供规避监管、绕过限制、攻击、滥用网络或其他违法违规用途的支持。
...
- 本脚本仅供学习网络协议、Linux 运维自动化和自有服务器链路管理，请遵守所在地法律法规。
